...
<script>
Chart.defaults.color='#64748b';Chart.defaults.borderColor='#e2e8f0';Chart.defaults.font.family="'Inter', sans-serif";Chart.defaults.animation=false;
window.chartsTotal = ${[ci, sw, vg?.transitions?.length, vg?.yearly_data?.length, results.crop_intensity_change, results.terrain?.total_area_ha].filter(Boolean).length};
window.chartsRendered = 0;
const onChartComplete = (chart) => {
