...
export function generateCSV(results) {
  ...
  if (results.vegetation) {
    ...
    if (results.vegetation.transitions) {
      ...
      for (const t of results.vegetation.transitions) {
        add(t.from ?? t.from_class ?? (t.label ? t.label.split('→')[0]?.trim() : ''), t.to ?? t.to_label ?? (t.label ? t.label.split('→')[1]?.trim() : ''), t.area_ha);
      }
    }
    add('');
  }
  ...
  if (results.crop_intensity_change) {
    ...
    for (const t of results.crop_intensity_change) {
      add(t.label ?? t.category ?? '', t.area_ha);
    }
    add('');
  }
...
<div style="width:300px;height:300px"><canvas id="terrainChart"></canvas></div>
<table style="flex:1"><thead><tr><th>Type</th><th>Area (ha)</th></tr></thead>
<tbody>${Object.entries(results.terrain).map(([k,v])=>`<tr><td>${k.replace(/_/g,' ')}</td><td>${v}</td></tr>`).join('')}</tbody></table>
</div></div>` : ''}
<div class="ft"><p>Generated by CSVAT — ${source}</p><p>${now}</p></div>
</div>
<script>
Chart.defaults.color='#64748b';Chart.defaults.borderColor='#e2e8f0';Chart.defaults.font.family="'Inter', sans-serif";
${ci ? `new Chart(document.getElementById('ciChart'),{type:'bar',data:{labels:${JSON.stringify(ciData.map(d=>d.year))},datasets:[{label:'Single Crop',data:${JSON.stringify(ciData.map(d=>d.single_crop_ha))},backgroundColor:'#22c55e',borderRadius:4},{label:'Double Crop',data:${JSON.stringify(ciData.map(d=>d.double_crop_ha))},backgroundColor:'#3b82f6',borderRadius:4},{label:'Triple Crop',data:${JSON.stringify(ciData.map(d=>d.triple_crop_ha))},backgroundColor:'#f59e0b',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#1e293b',font:{weight:500}}}},scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,title:{display:true,text:'Area (Hectares)'}}}}});` : ''}
${sw ? `new Chart(document.getElementById('swChart'),{type:'bar',data:{labels:${JSON.stringify(swData.map(d=>d.year))},datasets:[{label:'Kharif',data:${JSON.stringify(swData.map(d=>d.kharif_ha ?? d.seasonal_monsoon_ha ?? 0))},backgroundColor:'#14b8a6',borderRadius:4},{label:'Rabi',data:${JSON.stringify(swData.map(d=>d.rabi_ha ?? d.seasonal_winter_ha ?? 0))},backgroundColor:'#3b82f6',borderRadius:4},{label:'Zaid',data:${JSON.stringify(swData.map(d=>d.zaid_ha ?? d.perennial_ha ?? 0))},backgroundColor:'#93c5fd',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#1e293b',font:{weight:500}}}},scales:{x:{grid:{display:false}},y:{title:{display:true,text:'Area (Hectares)'}}}}});` : ''}
${vg?.transitions?.length ? `new Chart(document.getElementById('vgTransChart'),{type:'bar',data:{labels:${JSON.stringify(vg.transitions.filter(t=>(t.to??t.to_label)!=='Tree Cover'&&(t.to??t.to_label)!=='Forest').map(t=>(t.from??t.from_class??'')+(t.to??t.to_label ? ' → '+(t.to??t.to_label) : '')))},datasets:[{label:'Area (ha)',data:${JSON.stringify(vg.transitions.filter(t=>(t.to??t.to_label)!=='Tree Cover'&&(t.to??t.to_label)!=='Forest').map(t=>t.area_ha))},backgroundColor:['#ef4444','#f59e0b','#eab308','#9ca3af'],borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',scales:{x:{title:{display:true,text:'Area (Hectares)'}}},plugins:{legend:{display:false}}}});` : ''}
${vg?.yearly_data?.length ? `new Chart(document.getElementById('vgChart'),{type:'line',data:{labels:${JSON.stringify(vg.yearly_data.map(d=>d.year))},datasets:[{label:'Tree Cover (ha)',data:${JSON.stringify(vg.yearly_data.map(d=>d.tree_cover_ha))},borderColor:'#16a34a',backgroundColor:'rgba(22,163,74,0.1)',fill:true,borderWidth:3,tension:.3,pointRadius:4,pointBackgroundColor:'#16a34a'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#1e293b',font:{weight:500}}}},scales:{x:{grid:{display:false}},y:{title:{display:true,text:'Area (Hectares)'}}}}});` : ''}
${results.crop_intensity_change ? `new Chart(document.getElementById('cicChart'),{type:'bar',data:{labels:${JSON.stringify(results.crop_intensity_change.filter(t=>{const lbl=t?.label??t?.category;return lbl&&!lbl.includes('Total');}).map(t=>t.label??t.category??''))},datasets:[{label:'Area (ha)',data:${JSON.stringify(results.crop_intensity_change.filter(t=>{const lbl=t?.label??t?.category;return lbl&&!lbl.includes('Total');}).map(t=>t.area_ha))},backgroundColor:${JSON.stringify(results.crop_intensity_change.filter(t=>{const lbl=t?.label??t?.category;return lbl&&!lbl.includes('Total');}).map(t=>{const lbl=t.label??t.category??'';if(lbl.includes('Single To Double')||lbl.includes('Double To Triple')||lbl.includes('Single To Triple'))return'#22c55e';if(lbl.includes('Double To Single')||lbl.includes('Triple To Double')||lbl.includes('Triple To Single'))return'#ef4444';return'#3b82f6'}))},borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',scales:{x:{title:{display:true,text:'Area (Hectares)'}}},plugins:{legend:{display:false}}}});` : ''}
${results.terrain?.total_area_ha ? `new Chart(document.getElementById('terrainChart'),{type:'doughnut',data:{labels:${JSON.stringify(Object.entries(results.terrain).filter(([k])=>k!=='total_area_ha').filter(([,v])=>v>0).map(([k])=>k.replace(/_/g,' ')))},datasets:[{data:${JSON.stringify(Object.entries(results.terrain).filter(([k])=>k!=='total_area_ha').filter(([,v])=>v>0).map(([,v])=>v))},backgroundColor:['#f59e0b','#22c55e','#9ca3af','#ef4444','#3b82f6'],borderWidth:2,borderColor:'#fff'}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'right',labels:{color:'#1e293b',font:{weight:500}}}}}});` : ''}
</script></body></html>`;
}
